"""
XGBoost로 다음날 강수량(precip_tomorrow)을 예측한다.
16/17과 같은 입력 변수·같은 분할을 써서 세 모델을 나란히 비교할 수 있게 한다.
...
if __name__ == "__main__":
    ...
    model = XGBRegressor(n_estimators=N_TREES, random_state=SEED, n_jobs=-1)
    ...
    metrics.to_csv(OUTPUT_DIR / "xgb_regression_precip_metrics.csv", index=False)
    importances.to_csv(OUTPUT_DIR / "xgb_regression_precip_importances.csv", index=False)
