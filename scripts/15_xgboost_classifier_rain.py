"""
XGBoost 분류기로 다음날 강수 여부(rain_tomorrow)를 예측한다.
13/14와 같은 입력 변수·같은 분할을 써서 세 모델을 나란히 비교할 수 있게 한다.
...
if __name__ == "__main__":
    ...
    model = XGBClassifier(n_estimators=N_TREES, random_state=SEED, n_jobs=-1, eval_metric="logloss")
    ...
    metrics.to_csv(OUTPUT_DIR / "xgb_classifier_rain_metrics.csv", index=False)
    importances.to_csv(OUTPUT_DIR / "xgb_classifier_rain_importances.csv", index=False)
